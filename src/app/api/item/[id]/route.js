...
export async function PATCH(req, { params }) {
    ...
    try {
        const client = await getClientPromise();
        const db = client.db("sample_mflix");
        const existedData = await db.collection("movies").findOne({ _id: new ObjectId(id) });
        const updateData = { ...existedData, ...partialUpdate };
        const updatedResult = await db.collection("movies").updateOne({ _id: new ObjectId(id) }, { $set: updateData });
...
export async function PUT(req, { params }) {
    ...
    try {
        const client = await getClientPromise();
        const db = client.db("sample_mflix");
        const updatedResult = await db.collection("movies").updateOne({ _id: new ObjectId(id) }, { $set: data });
        return NextResponse.json(updatedResult, {
            status: 200,
            headers: corsHeaders
        })
